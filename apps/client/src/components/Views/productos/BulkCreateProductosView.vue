...
import { getSubcategorias, getCategoriaById } from '@/services/categorizacion.service';
...
// Helper: Resolve IDs
const transformAndResolveReferences = async (flatData) => {
    ...
    for (const name of subNames) {
        ...
        if (match) {
            // Found Subcategory. Now need its details (filters/options).
    ...
    for (const row of flatData) {
        ...
        if (subDetails && filtroName && opcionValue) {
            ...
            if (filtro) {
                ...
                if (opcion) {
                     // Check duplicate attributes for this product
                     if (!product.detalles.some(d => d.id_opcion_filtro === opcion.id)) {
