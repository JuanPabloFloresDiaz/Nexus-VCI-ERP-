<template>
  <v-dialog v-model="dialog" max-width="700px">
    <v-card>
      ...
      <v-card-text v-if="movimiento" class="pa-4">
        <v-row>
          <!-- Header Info -->
          <v-col cols="12">
            <div class="d-flex align-center justify-space-between">
              <h3 class="text-h6 text-primary">ID: {{ movimiento.id.slice(0, 8) }}...</h3>
              <v-chip class="font-weight-bold" :color="getTypeColor(movimiento.tipo_movimiento)">
          ...
          <!-- Notes -->
          <v-col v-if="movimiento.notas" cols="12">
            <v-alert color="info" density="compact" icon="mdi-note-text" variant="tonal">
