        ...
        <v-card>
            ...
            <v-card-text>
                ...
                <div v-else-if="availableFilters.length > 0">
                    <v-row>
                        <v-col cols="12" v-for="filtro in availableFilters" :key="filtro.id">
                            <v-select
            ...
            <v-card-actions>
                ...
                <v-btn color="blue-darken-1" variant="text" @click="confirm" :disabled="!isValid">Agregar</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
</template>
    ...
    if (val && props.product?.id_subcategoria) {
        ...
        if (res.data?.data) {
            ...
            if (props.product.detalles_filtros) {
                 props.product.detalles_filtros.forEach(df => {
